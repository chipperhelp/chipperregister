...
with st.form("register"):
    ...
    subbt=st.form_submit_button("Register now !")
    if subbt:
        if not email or not passw or not handle:
            ...
            try:
                ...
                st.success("Account created you may login now !")
                 ...
                 st.toast("🚫 Invalid email or user already exist's")



         
st.write('<div style="text-align: center;">Already on Chipper? <a href="https://chipper.streamlit.app">Log in</a></div>', unsafe_allow_html=True)
#http://localhost:8501/
